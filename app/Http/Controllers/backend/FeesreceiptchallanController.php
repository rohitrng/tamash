...
class FeesreceiptchallanController extends Controller
{
    public function index(){
        ...
        return view('backend.Fees-module.Fees_receipt_challan',compact('data_student_name','course_fees_head_orders_list_arr','late_fees_master','lumpsum_fees','siblings'));
    }
    ...
    public function get_student_info(Request $request){
       ...
       $course_fees_head_orders_list_arr = Course_fees_head_master::select('ac_head_name')->orderBy('order','ASC')->get();
        ...
        foreach($arr as $k => $ar){
            ...
            if('student_father_name' == $k) {
                $result['student_father_name'] = $ar;
                $data[] = [
                        $k => $ar,             
                ];
            }
    ...
    public function save_feesreceipt_challan(Request $request){
        ...
        if(empty($check)){
            $json = [
                'head_name' => (!is_string (implode(',', $request->post('head_name1')))),
